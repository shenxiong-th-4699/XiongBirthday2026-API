...
exports.saveDonate = functions.https.onRequest((req, res) => {
    ...
    try {
        ...
        res.status(200).json({ success: true, message: "บันทึกข้อมูลแล้ว" });
    } catch (error) {
...
exports.getDonationAll = functions.https.onRequest((req, res) => {
    cors(req, res, async () => {
        ...
        res.status(200).json({
            ...
            data: allData // จะเห็นครบทุกฟิลด์: bank, sender_account, ฯลฯ
        });
    } catch (error) {
        res.status(500).send(error.message);
    }
...
exports.getUserInfoByXid = functions.https.onRequest((req, res) => {
    ...
    try {
        ...
        // เรียงลำดับจากใหม่ไปเก่า (ทำในโค้ดเพื่อเลี่ยงการต้องสร้าง Composite Index บน Firestore)
